...
const DiningCartTab = lazy(() => import("../components/dining/DiningCartTab"));
const DiningReserveTab = lazy(() => import("../components/dining/DiningReserveTab"));
const DiningOrdersTab = lazy(() => import("../components/dining/DiningOrdersTab"));
...
const getImageUrl = (imagePath) => {
  if (!imagePath) return fallbackDiningImage;
...
const MenuCard = memo(function MenuCard({
  ...
  return (
    <div style={style} {...ariaAttributes}>
      <div className=" border border-luxe-border bg-white shadow-[0_18px_50px_rgba(28,28,28,0.06)] transition-all hover:shadow-[0_25px_60px_rgba(28,28,28,0.12)]group flex h-full max-w-[315px] mx-auto flex-col overflow-hidden rounded-[20px]">
        ...
        <div className="flex flex-1 flex-col p-3.5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <h3 className="mt-2 font-serif text-lg leading-tight">{item.name}</h3>
            </div>
          ...
          <div className="mt-auto flex items-center justify-between pt-5">
            ...
            {quantity > 0 ? (
              <span className="text-sm font-semibold text-luxe-bronze">Added to cart</span>
            ) : (
              <button
                type="button"
                onClick={() => onUpdateCart(item, 1)}
                disabled={!item.isAvailable}
                className="rounded-full bg-luxe-bronze px-3 py-1.5 text-sm font-semibold text-white transition hover:bg-luxe-charcoal disabled:cursor-not-allowed disabled:bg-luxe-muted"
              >
                Add
              </button>
            )}
...
export default function Dining() {
  ...
  const [visibleMenuItemCount, setVisibleMenuItemCount] = useState(INITIAL_VISIBLE_MENU_ITEMS);
  ...
  useEffect(() => {
    if (typeof window === "undefined") { setShouldLoadMenu(true); return undefined; }
    const rafId = window.requestAnimationFrame(() => setShouldLoadMenu(true));
  ...
  useEffect(() => {
    ...
    if (!protectedTabs.has(activeTab) || hasLoadedProtectedDataRef.current) return;
    loadProtectedData();
  ...
  const updateCart = useCallback((menuItem, delta) => {
    setCart((prev) => {
      ...
      if (!existing && delta > 0) return [...prev, { ...menuItem, quantity: 1 }];
      return prev
        .map((item) =>
  ...
  const handlePlaceOrder = async (event) => {
    ...
    setOrderMessage({ text: "", type: "" });
    if (!user) { setOrderMessage({ text: "Please log in to place a dining order.", type: "error" }); return; }
    if (cart.length === 0) { setOrderMessage({ text: "Add at least one menu item before placing an order.", type: "error" }); return; }
    if (orderForm.orderType === "Room Service" && !orderForm.roomNumber.trim()) { setOrderMessage({ text: "Room number is required for room service.", type: "error" }); return; }
    if (orderForm.orderType === "In-Restaurant" && !orderForm.tableNumber.trim()) { setOrderMessage({ text: "Table number is required for in-restaurant orders.", type: "error" }); return; }
    ...
    const newOrderPayload = {
      ...
      tableNumber: orderForm.orderType === "In-Restaurant" ? orderForm.tableNumber : undefined,
  ...
  const handleReserveTable = async (event) => {
    ...
    if (!reservationForm.tableNumber) { setReserveMessage({ text: "Please select a table.", type: "error" }); return; }
    if (!reservationForm.reservationTime) { setReserveMessage({ text: "Please select a reservation time.", type: "error" }); return; }
    ...
    try {
      ...
      setReservationForm({ tableNumber: "", reservationTime: "", guestsCount: 2, specialRequests: "" });
      const [tablesResponse, reservationsResponse] = await Promise.all([getDiningTables(), getMyDiningReservations()]);
      setTables(Array.isArray(tablesResponse?.data) ? tablesResponse.data : []);
      setReservations(Array.isArray(reservationsResponse?.data) ? reservationsResponse.data : []);
      hasLoadedProtectedDataRef.current = true;
      setActiveTab("orders");
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      ...
      if (cancelledOrder?._id) {
        setOrders((prev) => prev.map((order) => order._id === cancelledOrder._id ? cancelledOrder : order));
      }
    } catch (error) {
  ...
  const handleCancelReservation = async (resId) => {
    ...
    try {
      ...
      if (cancelledRes?._id) {
        setReservations((prev) => prev.map((res) => res._id === cancelledRes._id ? cancelledRes : res));
      }
  ...
  const canCancelReservation = useCallback((reservation) => {
    ...
    return cancellableStatuses.has(reservation.status) && new Date(reservation.reservationTime) > new Date();
  }, []);
  ...
  return (
    <div className="pb-14">
      <div className="mx-auto max-w-7xl px-4 pt-3 pb-8 lg:px-8" style={BELOW_THE_FOLD_STYLE}>
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none rounded-2xl border border-luxe-border bg-luxe-smoke py-3 pl-4 pr-10 text-sm font-semibold text-luxe-charcoal outline-none transition
                               focus:border-luxe-bronze focus:bg-white focus:ring-4 focus:ring-luxe-bronze/10
                               cursor-pointer lg:w-48"
                  >
                    {categories.map((cat) => (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-luxe-muted">Filters:</span>
...
>>>>>>> d942895ce6de60a7ef4db51895dc77c9bdd415fe
                ...
                <div className="rounded-[30px] border border-luxe-border bg-white p-6 shadow-[0_18px_50px_rgba(28,28,28,0.06)]">
                  ...
                  <div className="mt-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                    {Array.from({ length: 4 }).map((_, i) => (
                      <div key={i} className="overflow-hidden rounded-[20px] border border-luxe-border bg-white">
        ...
        {activeTab === "cart" && (
          <Suspense fallback={<Loader />}>
            <DiningCartTab
              cart={cart} clearCart={clearCart} formatCurrency={formatCurrency}
              getImageUrl={getImageUrl} handleOrderInputChange={handleOrderInputChange}
              handlePlaceOrder={handlePlaceOrder} inputClass={inputClass} isAdmin={isAdmin}
              orderForm={orderForm} orderMessage={orderMessage} submittingOrder={submittingOrder}
              tables={tables} totalAmount={totalAmount} updateCart={updateCart}
              setActiveTab={setActiveTab} user={user}
            />
          </Suspense>
        )}
        ...
        {activeTab === "reserve" && (
          <Suspense fallback={<Loader />}>
            <DiningReserveTab
              availableTables={availableTables} handleReservationInputChange={handleReservationInputChange}
              handleReserveTable={handleReserveTable} inputClass={inputClass} isAdmin={isAdmin}
              loadingProtectedData={loadingProtectedData} reservationForm={reservationForm}
              reserveMessage={reserveMessage} setReservationForm={setReservationForm}
              setActiveTab={setActiveTab} submittingReservation={submittingReservation} user={user}
            />
          </Suspense>
        )}
        ...
        {activeTab === "orders" && (
          <Suspense fallback={<Loader />}>
            <DiningOrdersTab
              canCancelReservation={canCancelReservation} canCancelOrder={canCancelOrder}
              formatCurrency={formatCurrency} handleCancelOrder={handleCancelOrder}
              handleCancelReservation={handleCancelReservation} isAdmin={isAdmin}
              loadingProtectedData={loadingProtectedData} orders={orders}
              reservations={reservations} statusStyles={statusStyles} user={user}
            />
          </Suspense>
        )}
      </div>
