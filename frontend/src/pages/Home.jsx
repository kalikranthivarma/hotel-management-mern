...
function CTABanner() {
  return (
    <section className="bg-[#FAFAF8] py-8 px-4">
  <div className="max-w-7xl mx-auto -mt-4">
    <div className="overflow-hidden rounded-[32px] bg-[linear-gradient(135deg,#1c1c1c_0%,#3d2b1f_100%)] px-8 md:px-14 py-10 md:py-12 text-white shadow-[0_24px_80px_rgba(28,28,28,0.18)]">

          <p className="text-xs font-bold uppercase tracking-[0.35em] text-[#c6a77d]">
            Start Your Journey
          </p>

        <h2 className="mt-3 max-w-4xl font-serif text-3xl sm:text-[4rem] leading-[1.1]">
            Experience the art of luxury travel
          </h2>

         <p className="mt-4 max-w-2xl text-base sm:text-lg leading-8 text-white/75">
...
function Footer() {
  ...
  return (
    <footer className="bg-luxe-charcoal text-[#c9bda8] font-[Cormorant_Garamond,Georgia,serif]">

      <div className="h-px w-full bg-gradient-to-r from-transparent via-luxe-bronze/50 to-transparent" />

      <div className="mx-auto max-w-7xl px-6 sm:px-10 lg:px-16">

        <div className="grid grid-cols-2 gap-x-8 gap-y-12 py-16 md:grid-cols-3 lg:grid-cols-[1.8fr_1fr_1fr_1fr_1fr] border-b border-luxe-bronze/[0.15]">

          <div className="col-span-2 md:col-span-3 lg:col-span-1">
            <div className="mb-5">
              <p className="font-[Playfair_Display,Georgia,serif] text-[1.35rem] tracking-[0.16em] text-[#f0e8d5] font-semibold m-0">
                KNSU STAYS
              </p>
              <p className="mt-1 text-[0.6rem] tracking-[0.32em] uppercase text-[#6e6559] m-0">
                Modern Boutique Collection
              </p>
            </div>

            <div className="mb-5 h-px w-9 bg-luxe-bronze/40" />

            <p className="mb-7 max-w-[240px] text-[0.9rem] leading-[1.8] text-[#7a7166] font-light">
              A world of curated luxury experiences across India's most iconic destinations.
            </p>

            <div className="flex gap-2.5">
              {socials.map(({ Icon, title }) => (
                <button key={title} className="flex h-[44px] w-[44px] items-center justify-center rounded-full border border-luxe-bronze/30 text-luxe-bronze">
                  <Icon />
                </button>
              ))}
            </div>
          ...
          {cols.map((col) => (
            <div key={col.title} className="flex flex-col gap-3.5">
              <p className="mb-1 text-[0.82rem] tracking-[0.28em] uppercase text-luxe-bronze m-0">
                {col.title}
              </p>

              {col.links.map(([label, href]) =>
                href.startsWith("/") ? (
                  <Link key={label} to={href} className="text-[#c2b49a] hover:text-luxe-bronze">
                    {label}
                  </Link>
                ) : (
                  <a key={label} href={href} className="text-[#c2b49a] hover:text-luxe-bronze">
                    {label}
                  </a>
                )
              )}
        ...
        <div className="flex flex-col items-center justify-between gap-4 py-6 sm:flex-row">
          ...
          <div className="flex gap-6">
            {["Privacy Policy", "Terms of Use", "Cookie Policy"].map((l) => (
              <a key={l} href="#" className="text-[0.75rem] text-[#4a4540] hover:text-[#c2b49a]">
                {l}
              </a>
            ))}
          </div>
