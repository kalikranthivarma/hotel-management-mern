...
const ResendVerification = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(submitError.response?.data?.message || submitError.message || "Failed to send verification link.");
    } finally {
  ...
  return (
    <section className="mx-auto flex min-h-[calc(100vh-88px)] w-full max-w-3xl items-center px-4 py-10">
      ...
      >
        <p className="text-xs font-bold uppercase tracking-[0.35em] text-luxe-bronze">Verification Expired</p>
        <h1 className="mt-4 font-serif text-4xl sm:text-5xl">Resend link</h1>
